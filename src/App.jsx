...
/* ===================== root ===================== */
export default function App({ forcedRole = null, isOwner = false, ownerEmail = "" } = {}) {
  const [stage, setStage] = useState(forcedRole ? "app" : "landing");
  const [role, setRole] = useState(forcedRole || null);
  const [trial, setTrial] = useState(null);
  const [plan, setPlan] = useState(null);
  const [ready, setReady] = useState(false);
  useEffect(() => { (async () => { try { const r = await window.storage?.get("cura_session"); if (r?.value) { const s = JSON.parse(r.value); if (!forcedRole && s.role) { setRole(s.role); setStage("app"); } } } catch (e) {} try { const t = await window.storage?.get("qura_trial"); if (t?.value) setTrial(JSON.parse(t.value)); } catch (e) {} try { const pl = await window.storage?.get("qura_plan"); if (pl?.value) setPlan(JSON.parse(pl.value)); } catch (e) {} setReady(true); })(); }, []);
  ...
  return (
    <div className="cura">
      ...
      {stage === "app" && role && <Shell role={role} trial={trial} plan={plan} onPlan={choosePlan} onExtend={extendTrial} onSignup={() => setStage("signup")} onLogout={logout} onHome={home} onSwitch={switchRole} isOwner={isOwner} ownerEmail={ownerEmail} />}
    </div>
  );
}
