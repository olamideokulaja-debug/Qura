    ...
    <div style={{ background: "#fff" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 30, background: "rgba(255,255,255,.82)", backdropFilter: "blur(12px)", borderBottom: "1px solid var(--line)" }}>
        <div className="wrap row" style={{ justifyContent: "space-between", height: 72 }}>
          ...
          <div className="row" style={{ gap: 12 }}><button className="btn btn-light hsm" style={{ background: "var(--bg)" }} onClick={onEnter}>Sign in</button><button className="btn btn-primary" onClick={onDemo}>Book a demo</button></div>
        </div>
      </div>
      ...
      <div style={{ background: "radial-gradient(115% 85% at 50% -8%, #E6F4F2 0%, #F3F9FD 44%, #fff 100%)", borderBottom: "1px solid var(--line)", position: "relative", overflow: "hidden" }}>
        <div className="wrap" style={{ padding: "92px 24px 70px", textAlign: "center" }}>
          ...
          <div className="row reveal" style={{ gap: 14, justifyContent: "center", marginTop: 34, flexWrap: "wrap" }}><button className="btn lift" style={{ padding: "15px 28px", fontSize: 16, background: "var(--blue)", color: "#fff", boxShadow: "0 6px 18px rgba(45,107,255,.32)" }} onClick={onEnter}>Enter the platform <ArrowRight size={18} /></button><a href="#founders" className="btn btn-dark lift" style={{ padding: "15px 28px", fontSize: 16 }}>Meet the founders</a></div>
      ...
      <div id="pricing" style={{ background: "var(--navy)" }}>
        <div className="wrap" style={{ padding: "80px 24px", textAlign: "center" }}>
          <Reveal>
            ...
            <div className="row" style={{ gap: 14, justifyContent: "center", marginTop: 34, flexWrap: "wrap" }}><button className="btn btn-blue lift" style={{ padding: "15px 28px", fontSize: 16 }} onClick={onEnter}>Enter the platform <ArrowRight size={18} /></button><button onClick={onDemo} className="btn btn-dghost" style={{ padding: "15px 28px", fontSize: 16 }}><Mail size={17} /> Talk to us</button></div>
          </Reveal>
        </div>
      </div>
...
function Shell({ role, onLogout, onHome, onSwitch, trial, onSignup, plan, onPlan, onExtend }) {
  const nav = NAVS[role];
...
export default function App() {
  ...
  useEffect(() => { (async () => { try { const r = await window.storage?.get("cura_session"); if (r?.value) { const s = JSON.parse(r.value); if (s.role) { setRole(s.role); setStage("app"); } } } catch (e) {} try { const t = await window.storage?.get("qura_trial"); if (t?.value) setTrial(JSON.parse(t.value)); } catch (e) {} try { const pl = await window.storage?.get("qura_plan"); if (pl?.value) setPlan(JSON.parse(pl.value)); } catch (e) {} setReady(true); })(); }, []);
  const enter = (rk) => { setRole(rk); setStage("app"); try { window.storage?.set("cura_session", JSON.stringify({ role: rk })); } catch (e) {} };
  ...
  const switchRole = (rk) => { setRole(rk); try { window.storage?.set("cura_session", JSON.stringify({ role: rk })); } catch (e) {} };
  const logout = () => { setRole(null); setStage("landing"); try { window.storage?.delete("cura_session"); } catch (e) {} try { window.__quraSignOut && window.__quraSignOut(); } catch (e) {} };
  ...
  return (
    <div className="cura">
      <style>{STYLES}</style>
      {stage === "landing" && <Landing onEnter={() => setStage("login")} onDemo={() => setStage("demo")} />}
      {stage === "demo" && <DemoBooking onHome={home} onSignIn={() => setStage("login")} />}
      {stage === "signup" && <Signup onHome={home} onSignIn={() => setStage("login")} onChoose={(pl, annual) => { choosePlan(pl, annual); setStage("login"); }} />}
      {stage === "login" && <Login onNext={enter} onHome={home} onSignup={() => setStage("signup")} />}
      {stage === "role" && <RoleSelect onPick={enter} />}
      {stage === "app" && role && <Shell role={role} trial={trial} plan={plan} onPlan={choosePlan} onExtend={extendTrial} onSignup={() => setStage("signup")} onLogout={logout} onHome={home} onSwitch={switchRole} />}
    </div>
  );
}
